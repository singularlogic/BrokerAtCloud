	...
	public void printMinMax() {
		for (Object i : JsonRoot.INSTANCE.offerings) {
			...
			for (Map<String, Object> var : varspace) {
				System.out
						.println(String.format("fakeRange.put(\"%s\", %.1f);", var.get(JsonRoot.VarName) + "Max", 0.0));
				System.out
						.println(String.format("fakeRange.put(\"%s\", %.1f);", var.get(JsonRoot.VarName) + "Min", 0.0));
				System.out.println("");
			}
		}
	}
	...
	public Object get(String service, String attribute) {
		...
		for (Map<String, Object> var : varspace) {
			if (attribute.equals(var.get(JsonRoot.VarName))) {
				...
				if (value != null)
					return convert(value.toString(), var.get(JsonRoot.VarName).toString(), isAscent);
