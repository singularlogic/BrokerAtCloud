 ...
 * <em><b>Top Down Visitor</b></em>'. <!-- end-user-doc -->
 * <p>
 * </p>
 *
 * @generated
 */
public abstract class TopDownVisitorImpl<ContextType, ResultType> extends EObjectImpl
		implements TopDownVisitor<ContextType, ResultType> {
	/**
	...
	public ResultType visitVariabilityModel(final VariabilityModel node, final ContextType context) {
		for (int i = 0; i < node.getContext().size(); i++) {
			((diva.Variable) node.getContext().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getProperty().size(); i++) {
			((diva.Property) node.getProperty().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getDimension().size(); i++) {
			((diva.Dimension) node.getDimension().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getRule().size(); i++) {
			((diva.Rule) node.getRule().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getConstraint().size(); i++) {
			((diva.Constraint) node.getConstraint().get(i)).accept(this, context);
		}
		if (node.getSimulation() != null) {
			node.getSimulation().accept(this, context);
		}
		return null;
	...
	public ResultType visitInvariant(final Invariant node, final ContextType context) {
		for (int i = 0; i < node.getAnnotation().size(); i++) {
			((diva.Annotation) node.getAnnotation().get(i)).accept(this, context);
		}
	...
	public ResultType visitEnumVariable(final EnumVariable node, final ContextType context) {
		...
		for (int i = 0; i < node.getLiteral().size(); i++) {
			((diva.EnumLiteral) node.getLiteral().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitAndTerm(final AndTerm node, final ContextType context) {
		for (int i = 0; i < node.getTerm().size(); i++) {
			((diva.Term) node.getTerm().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getAnnotation().size(); i++) {
			((diva.Annotation) node.getAnnotation().get(i)).accept(this, context);
		}
	...
	public ResultType visitNotTerm(final NotTerm node, final ContextType context) {
		...
		if (node.getTerm() != null) {
			node.getTerm().accept(this, context);
		}
		return null;
	...
	public ResultType visitVariant(final Variant node, final ContextType context) {
		...
		for (int i = 0; i < node.getPropertyValue().size(); i++) {
			((diva.PropertyValue) node.getPropertyValue().get(i)).accept(this, context);
		}
		if (node.getDependency() != null) {
			node.getDependency().accept(this, context);
		}
		if (node.getAvailable() != null) {
			node.getAvailable().accept(this, context);
		}
		if (node.getRequired() != null) {
			node.getRequired().accept(this, context);
		}
		return null;
	...
	public ResultType visitDimension(final Dimension node, final ContextType context) {
		...
		for (int i = 0; i < node.getVariant().size(); i++) {
			((diva.Variant) node.getVariant().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getConstraints().size(); i++) {
			((diva.MultiplicityConstraint) node.getConstraints().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitPriorityRule(final PriorityRule node, final ContextType context) {
		...
		if (node.getContext() != null) {
			node.getContext().accept(this, context);
		}
		for (int i = 0; i < node.getPriority().size(); i++) {
			((diva.PropertyPriority) node.getPriority().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitProperty(final Property node, final ContextType context) {
		...
		for (int i = 0; i < node.getLiteral().size(); i++) {
			((diva.PropertyLiteral) node.getLiteral().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitSimulationModel(final SimulationModel node, final ContextType context) {
		...
		for (int i = 0; i < node.getScenario().size(); i++) {
			((diva.Scenario) node.getScenario().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitContext(final Context node, final ContextType context) {
		...
		for (int i = 0; i < node.getVariable().size(); i++) {
			((diva.VariableValue) node.getVariable().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getConfiguration().size(); i++) {
			((diva.Configuration) node.getConfiguration().get(i)).accept(this, context);
		}
		if (node.getOracle() != null) {
			node.getOracle().accept(this, context);
		}
		for (int i = 0; i < node.getPriority().size(); i++) {
			((diva.Priority) node.getPriority().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitConfiguration(final Configuration node, final ContextType context) {
		for (int i = 0; i < node.getScore().size(); i++) {
			((diva.Score) node.getScore().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getAnnotation().size(); i++) {
			((diva.Annotation) node.getAnnotation().get(i)).accept(this, context);
		}
		for (int i = 0; i < node.getVariant().size(); i++) {
			((diva.ConfigVariant) node.getVariant().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitScenario(final Scenario node, final ContextType context) {
		...
		for (int i = 0; i < node.getContext().size(); i++) {
			((diva.Context) node.getContext().get(i)).accept(this, context);
		}
		return null;
	...
	public ResultType visitConfigurationModel(final ConfigurationModel node, final ContextType context) {
		for (int i = 0; i < node.getConfigurations().size(); i++) {
			((diva.SuitableConfiguration) node.getConfigurations().get(i)).accept(this, context);
		}
		return null;
