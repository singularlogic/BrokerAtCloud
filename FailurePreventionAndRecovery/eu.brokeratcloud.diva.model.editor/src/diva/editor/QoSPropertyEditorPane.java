...
public class QoSPropertyEditorPane extends DiVATableEditorPane implements Adapter {
	...
	public QoSPropertyEditorPane(DiVATableEditor editor, IWorkbenchPage page, IWorkbenchPart part) {
	...
	protected void createPropertyColumns(TreeViewer viewer, VariabilityModel m) {
		...
		column.setWidth(100);
		viewerColumn
				.setLabelProvider(new DiVALabelProvider(new AdapterFactoryLabelProvider(editor.getAdaptedFactory())));
	...
	// static String[] QosValues = {"-", "Very Low", "Low", "Medium", "High",
	// "Very High" };
	...
	public class QoSPropertyEditingSupport extends EditingSupport {
		...
		protected CellEditor getCellEditor(Object element) {
			...
			if (element instanceof Variant) {
				...
				for (int i = 0; i < property.getLiteral().size(); i++)
					choices[i] = property.getLiteral().get(i).getName();
			...
			if (element instanceof Variant) {
				...
				if (pv != null) {
					...
					for (res = 0; res < property.getLiteral().size(); res++) {
						if (property.getLiteral().get(res).getValue() == pv.getValue())
							break;
					}
					result = res;
				} else
		...
		protected void setValue(Object element, Object value) {
			...
			if (element instanceof Variant) {
				...
				if (pv != null) {
					...
					SetCommand cmd = new SetCommand(editor.getEditingDomain(), pv,
							DivaPackage.eINSTANCE.getPropertyValue_Value(), property.getLiteral().get(idx).getValue());
		...
		public String getText(Object element) {
			...
			if (element instanceof Variant) {
				...
				if (v.getType().getProperty().contains(property)) {
					...
					// else if (pv.getValue()>=QosValues.length)
					// pv.setValue(QosValues.length-1);
					if (!pv.eAdapters().contains(QoSPropertyEditorPane.this))
						pv.eAdapters().add(QoSPropertyEditorPane.this);

					// search in the property
					for (PropertyLiteral l : property.getLiteral()) {
						if (l.getValue() == pv.getValue())
							return l.getName();
					}
