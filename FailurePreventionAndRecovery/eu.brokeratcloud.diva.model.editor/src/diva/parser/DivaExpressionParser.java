...
public class DivaExpressionParser extends antlr.LLkParser implements DivaExpressionParserTokenTypes {
				...
				if (s instanceof BooleanVariable) {
					...
					if (value != null)
						throw new RecognitionException(
								"Sensor " + name + " is not an enumeration, cannot compare it to " + value);
				} else if (s instanceof EnumVariable) {
					if (value == null)
						throw new RecognitionException(
								"Sensor " + name + " is an enumeration, it should be compared with a literal");
					...
					if (est.getValue() == null)
						throw new RecognitionException("Sensor " + name + " does not contains value " + value);
					t = est;
				}
			} else if (variants.get(name) != null) {
				if (value != null)
					throw new RecognitionException(
							"Variant " + name + " is not an enumeration, cannot compare it to " + value);
	...
	public static final String[] _tokenNames = { "<0>", "EOF", "<2>", "NULL_TREE_LOOKAHEAD", "OR", "AND", "NOT",
			"OPEN_PAREN", "CLOSE_PAREN", "ID", "EQUALS", "DOT", "LCURLY", "RCURLY", "LBRAK", "RBRAK", "PLUS", "MINUS",
			"DOLLAR", "COL_COL", "SCOL", "AS", "CREATE", "TRUE", "FALSE", "ARROW", "COLON", "COMMA", "STRING_LITERAL",
			"CHAR_LITERAL", "ESC", "INT_LITERAL", "REAL_LITERAL", "EXPONENT", "DIGIT", "WS", "SINGLE_LINE_COMMENT",
			"CONTEXT_MULTI_LINE_COMMENT", "EMPTY_LINE_COMMENT", "MULTI_LINE_COMMENT" };
	...
	public static Term parse(VariabilityModel model, String exp) throws Exception {
		...
		if (p.hasErrors())
			throw new Exception(p.errors.get(0));
		return result;
	}
