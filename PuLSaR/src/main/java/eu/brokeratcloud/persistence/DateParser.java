/*
 *  RETRIEVED FROM: http://www.koders.com/java/fidDBC85D14D02AA458CE8B8A25256E176EAC6EA748.aspx
 */
    ...
    public static Date parseW3CDateTime(String sDate) {
        ...
        if (tIndex>-1) {
            ...
            if (tzdIndex>-1) {
				...
				if (secFraction==-1) { secFraction = pre.indexOf("."); }
                if (secFraction>-1) {
                    pre = pre.substring(0,secFraction);
                }
                String post = sDate.substring(tzdIndex);
				if (post.indexOf(":")==-1) {
					post = post.substring(0, 3) + ":" + post.substring(3, 5);
				}
                sDate = pre + "GMT" + post;
            }
        }
