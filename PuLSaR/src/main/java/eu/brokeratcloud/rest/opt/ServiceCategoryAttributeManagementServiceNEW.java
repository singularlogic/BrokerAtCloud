...
/*	public static void main(String[] args) throws Exception {
		// Cleaning existing repository data...
		...
		client.execute("DELETE { ?s ?p ?o } WHERE { ?s ?p ?o . FILTER REGEX(str(?s),'http://www.broker-cloud.eu/service-descriptions/CAS/broker#','') . FILTER (!regex(str(?s),'#CAS','')) . FILTER (!regex(str(?s),'dependsOn','')) . FILTER (!regex(str(?s),'App','')) . }");
		client.execute("DELETE { ?s ?p ?o } WHERE { ?s ?p ?o . FILTER REGEX(str(?s),'usdl-pref#','') }");
		client.execute("DELETE { ?s ?p ?o } WHERE { ?s ?p ?o . FILTER REGEX(str(?p),'http://www.broker-cloud.eu/service-descriptions/CAS/broker#has','') }");
		System.out.println("ok");
		
		ServiceCategoryAttributeManagementServiceNEW debug = new ServiceCategoryAttributeManagementServiceNEW();
		
		// Testing Service Attribute creation
		ServiceCategoryAttribute sca = new ServiceCategoryAttribute();
		sca.setServiceCategory("google-map");		// or <http://www.broker-cloud.eu/service-descriptions/CAS/categories#google-map>
		sca.setAttribute("attr-performance");		// or <http://www.brokeratcloud.eu/v1/opt/SERVICE-ATTRIBUTE#attr-performance>
		sca.setType("NUMERIC_DEC");
		sca.setMandatory(true);
		sca.setUnit("P1");
		sca.setMin(95);
		sca.setMax(100);
		sca.setFmin(new eu.brokeratcloud.opt.type.TFN(45, 60, 75));
		sca.setFmax(new eu.brokeratcloud.opt.type.TFN(65, 80, 95));
		String[] term = new String[5];
		term[0]="VERY LOW"; term[1]="LOW"; term[2]="OK"; term[3]="HIGH"; term[4]="VERY HIGH";
		sca.setTerms(term);
		sca.setMembers(term);
		System.out.println("SCA TO CREATE = "+sca);
		debug.stopDebug("BEFORE 1st CREATION");
		String scaId = _createServiceCategoryAttribute(sca);
		debug.stopDebug("AFTER 1st CREATION");
		
		// Testing Service Attribute retrieval
		sca = null;
		debug.stopDebug("BEFORE 1st RETRIEVAL");
		sca = _getServiceCategoryAttribute( scaId );
		System.out.println("SCA RETRIEVED = "+sca);
		debug.stopDebug("AFTER 1st RETRIEVAL");
		
		scaId = sca.getId();
		
		// Testing Service Attribute retrieval for Classification Dimensions
		debug.stopDebug("BEFORE CLASSIF.DIM. SCA RETRIEVAL");
		ServiceCategoryAttribute[] scaList = _getServiceCategoryAttributes( sca.getServiceCategory() );
		System.out.println("SCA-LIST RETRIEVED = "+java.util.Arrays.deepToString(scaList));
		debug.stopDebug("AFTER CLASSIF.DIM. SCA RETRIEVAL");
		
		// Testing Service Attribute creation with same ID's (MUST FAIL)
		sca = new ServiceCategoryAttribute();
		sca.setServiceCategory("google-map");		// or <http://www.broker-cloud.eu/service-descriptions/CAS/categories#google-map>
		sca.setAttribute("attr-performance");		// or <http://www.brokeratcloud.eu/v1/opt/SERVICE-ATTRIBUTE#attr-performance>
		...
		sca.setMin(95);
		sca.setMax(100);
		System.out.println("SCA TO CREATE (duplicate) = "+sca);
		debug.stopDebug("BEFORE 2nd CREATION");
		try {
			_createServiceCategoryAttribute(sca);
		} catch (Exception e) {
			System.err.println("SCA CREATION FAILED, as planned: "+e);
		}
		debug.stopDebug("AFTER 2nd CREATION");
		
		// Testing Service Attribute update
		sca = new ServiceCategoryAttribute();
		sca.setId( scaId );
		sca.setServiceCategory("google-map");		// or <http://www.broker-cloud.eu/service-descriptions/CAS/categories#google-map>
		sca.setAttribute("attr-financial-cost");	// or <http://www.brokeratcloud.eu/v1/opt/SERVICE-ATTRIBUTE#attr-cost>
		sca.setType("NUMERIC_DEC");
		sca.setMandatory(true);
		sca.setUnit("P1");
		sca.setMin(90);
		sca.setMax(100);
		System.out.println("SCA TO UPDATE (new values) = "+sca);
		debug.stopDebug("BEFORE 1st UPDATE");
		scaId = _updateServiceCategoryAttribute(sca);
		debug.stopDebug("AFTER 1st UPDATE");
		
		scaId = sca.getId();
		
		// Testing Service Attribute retrieval (2nd time)
		sca = null;
		debug.stopDebug("BEFORE 2nd RETRIEVAL");
		sca = _getServiceCategoryAttribute( scaId );
		System.out.println("SCA RETRIEVED = "+sca);
		debug.stopDebug("AFTER 2nd RETRIEVAL");
		
		// Testing Service Attribute delete
		sca = null;
		System.out.println("SCA TO DELETE = "+scaId);
		debug.stopDebug("BEFORE 1st DELETE");
		_deleteServiceCategoryAttribute(scaId);
		debug.stopDebug("AFTER 1st DELETE");
		
		// Testing Service Attribute retrieval (Not found!!!)
		sca = null;
		debug.stopDebug("BEFORE 3rd RETRIEVAL");
		try {
			sca = _getServiceCategoryAttribute(scaId);
			System.out.println("SCA RETRIEVED = "+sca);
		} catch (Exception e) {
			System.err.println("SCA RETRIEVAL FAILED, as planned: "+e);
		}
		debug.stopDebug("AFTER 3rd RETRIEVAL");
	}*/
...
//						  " ?s  <"+btUri+">  <"+cdUri+"> .  ?s  <"+rtUri+">  ?oa . ?oa <http://purl.org/dc/terms/title> ?name } ORDER BY ?name ";
...
//		String oaId = pv.getRefToServiceAttribute().getId();
//		if (oaId==null || oaId.trim().isEmpty()) { logger.trace("getServiceCategoryAttributeFromPreference: END: oaId is null or empty"); return null; }
...
//		sca = _prepareServiceCategoryAttribute(oaId, bpp, apv, pv, individuals, topIndividual);
...
//XXX:DELETE:2014-12-18:
/* 		logger.trace("createServiceCategoryAttribute: Querying for Broker Policy instance...");
		Object tmpBP = client.queryValue("SELECT DISTINCT ?s WHERE { ?s a <"+serviceModel+"> }");
		String bpInstance = tmpBP!=null ? tmpBP.toString() : null;
 		logger.trace("createServiceCategoryAttribute: Broker Policy instance={}", bpInstance);
		if (bpInstance==null) {
			throw new Exception("createServiceCategoryAttribute: Broker Policy instance NOT FOUND");
		}*/
		...
		// Add new service property to broker policy instance
//XXX:DELETE:2014-12-18:
/*		if (apv!=null) {
			pm.insertRdfTriple(bpInstance, pm.getObjectUri(bpp), pm.getObjectUri(apv));
		} else
		if (datatype!=null && !datatype.trim().isEmpty()) {
			pm.insertRdfTriple(bpInstance, pm.getObjectUri(bpp), datatype);
		}*/
	...
	protected static void _deleteServiceCategoryAttribute(String id) throws Exception {
		...
		if (client==null) client = SparqlServiceClientFactory.getClientInstance();
//XXX:DELETE:2014-12-18:
/* 		logger.trace("deleteServiceCategoryAttribute: Querying for Broker Policy instance...");
		Object tmpBP = client.queryValue("SELECT DISTINCT ?s WHERE { ?x <http://www.w3.org/2000/01/rdf-schema#subClassOf> <http://www.linked-usdl.org/ns/usdl-core#ServiceModel> . ?s a ?x }");
		String bpInstance = tmpBP!=null ? tmpBP.toString() : null;
 		logger.trace("deleteServiceCategoryAttribute: Broker Policy instance={}", bpInstance);
		if (bpInstance==null) {
			throw new Exception("deleteServiceCategoryAttribute: Broker Policy instance NOT FOUND");
		}*/
		// Delete service property from broker policy instance
//XXX:DELETE:2014-12-18:
/*		if (po.apv!=null) {
			pm.removeRdfTriple(bpInstance, pm.getObjectUri(po.bpp), pm.getObjectUri(po.apv));
		} else
		if (po.datatype!=null && !po.datatype.trim().isEmpty()) {
			pm.removeRdfTriple(bpInstance, pm.getObjectUri(po.bpp), po.datatype);
		}*/
