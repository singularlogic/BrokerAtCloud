...
/*	@GET
	@POST
	@Path("/get-broker-policy-attributes/")
	@Produces("application/json")
	@RolesAllowed("admin")
	public String getBrokerPolicyAttributes(@Context HttpServletRequest request) throws IOException {
		long startTm = System.currentTimeMillis();
		logger.info("-------------- getBrokerPolicyAttributes: INPUT: n/a");
		
		// Call REST service in order to get attribute list
		ResteasyClient client = new ResteasyClientBuilder().build();
		ResteasyWebTarget target = client.target(baseUrl+"/opt/attributes/broker-policy");
		long callStartTm = System.currentTimeMillis();
		Response response = target.request().get();
		OptimisationAttribute[] atList = response.readEntity( OptimisationAttribute[].class );
		long callEndTm = System.currentTimeMillis();
		...
		logger.info("-------------- getBrokerPolicyAttributes: OUTPUT: {}", str);
		long endTm = System.currentTimeMillis();
		logger.debug("duration={}ms   contacting WS: {}ms", endTm-startTm, callEndTm-callStartTm);
		return str;
	}*/
...
//		logger.debug("Is Cascade Delete: {}", cascade);
	...
	public Response createAttribute(OptimisationAttribute atIn) throws IOException {
		...
		// Set parent
		/*String parent = atIn.getParent();
		if (parent!=null && !(parent=parent.trim()).isEmpty()) {
			RdfSubject subject = OptimisationAttribute.class.getAnnotation(RdfSubject.class);
			if (subject!=null) {
				String uri = subject.uri();
				if (uri!=null && !parent.startsWith(uri)) {
					parent = uri+"#"+parent;
					atIn.setParent(parent);
					logger.debug("Parent set to:  {}", parent);
				}
			}
		}*/
...
//				sb.append( String.format(fmtText, "No, Yes") );
			} else 
...
/*	//2014-11-21: I18n: Alternative way to return multilingual strings - NOT USED
	// Usage: return new MyStreamingOutput(str);
	protected static class MyStreamingOutput implements javax.ws.rs.core.StreamingOutput {
		protected String str;
		public MyStreamingOutput(String s) { str = s; }
		public void write(java.io.OutputStream outputStream) throws IOException { //, WebApplicationException {
			java.io.PrintStream writer = new java.io.PrintStream(outputStream, true, "UTF-8");
			writer.println(str);
		}
	}*/
	...
	@Produces("application/json;charset=UTF-8")
	@RolesAllowed("admin")
	public /*Response*/ String saveServiceCategoryAttributes(@PathParam("cat_id") String catId, GridItem item) throws IOException {
		long startTm = System.currentTimeMillis();
		...
		if (item==null || ServiceCategoryAttribute.isUnknownType(item.getType())) {
			//return Response.status( 200 ).entity("Unknown type").build();
		...
		if (item.getFrom()!=null && !item.getFrom().trim().isEmpty()) {
			//if (type.equals("NUMERIC_INC") || type.equals("NUMERIC_DEC") || type.equals("NUMERIC_RANGE")) {
