...
/*			ClassificationDimension category = null;
			try {
				if (prIn.getServiceCategory()!=null && !prIn.getServiceCategory().trim().isEmpty()) category = (ClassificationDimension)pm.find( prIn.getServiceCategory().trim(), ClassificationDimension.class );
			} catch (Exception e) { logger.error("_transcribe: Service category not found: {}", prIn.getServiceCategory()); return null; }
			if (category==null) category = profile.getServiceCategory();
			if (category==null) logger.error("_transcribe: Service Category is NULL: profile={}", prIn.getId());*/
...
//		if (profile.getServiceCategory()!=null) prOut.setServiceCategory( profile.getServiceCategory().getId() );
	...
	public CPP saveProfile(CPP prIn) throws IOException {
		...
		if (id==null || id.trim().isEmpty()) {
			// Fail...
			throw new IllegalArgumentException("Invalid Profile Id : "+prIn);
		}*/
...
// TODO: TEMP CODE BEGIN - call Governance WS to retrieve service categories (dependency on SEERC's work ???)
									long callStartTm=System.currentTimeMillis();
									List<Object> list = null;
									try {
										eu.brokeratcloud.persistence.RdfPersistenceManager pm = eu.brokeratcloud.persistence.RdfPersistenceManagerFactory.createRdfPersistenceManager();
										list = pm.findAll(ServiceCategory.class);
										logger.debug("{} Service Categories found", list.size());
									} catch (Exception e) {
										e.printStackTrace();
										logger.error("{}: Returning an empty array of {}", getClass().getName(), ServiceCategory.class);
									}
									long callEndTm=System.currentTimeMillis();
									boolean first = true;
									StringBuilder sb = new StringBuilder("[ ");
									if (list!=null) {
										for (Object o : list) {
											ServiceCategory sc = (ServiceCategory)o;
											String id = sc.getId();
											String text = sc.getName();
											String parent = "#";
											if (sc.getParent()!=null) parent = sc.getParent().getId();
											String opt = parent.equals("#") ? ", 'state':{ 'opened':true, 'selected':true }" : "";
											if (first) first=false; else sb.append(", ");
											sb.append( String.format("{ 'id':'%s', 'text':'%s', 'parent':'%s' %s }", id, text, parent, opt) );
										}
									}
									sb.append(" ]");
									String str = sb.toString().replace("'", "\"");
// TODO: TEMP CODE END - call Governance WS to retrieve service categories (dependency on SEERC's work ???)
...
//											else { parent = sc.getInScheme().getId(); if (parent==null) { parent = pm.getObjectUri(sc.getInScheme()); sc.getInScheme().setId(parent); } if (!dimList.contains(sc.getInScheme())) dimList.add(sc.getInScheme()); }
											else { parent = "#"; }
											if (first) first=false; else sb.append(", ");
											sb.append( String.format("{ 'id':'%s', 'text':'%s', 'parent':'%s' }", id, text, parent) );
										}
										// Process classification dimension schemes
//										String parent = "__SRV_CLSS_ROOT__";
/*										String parent = "#";
										String opt = ", 'state':{ 'opened':true, 'selected':true }";
										for (Object o : dimList) {
											ClassificationDimensionScheme sc = (ClassificationDimensionScheme)o;
											String id = sc.getId();
											String text = sc.getPrefLabel();
											if (first) first=false; else sb.append(", ");
											sb.append( String.format("{ 'id':'%s', 'text':'%s', 'parent':'%s', 'icon':'/forms/admin/images/classification-dimension-node.png' %s }", id, text, parent, opt) );
										}
*/
/*										// Add tree root
										if (first) first=false; else sb.append(", ");
										sb.append( String.format("{ 'id':'__SRV_CLSS_ROOT__', 'text':'Service Classification Root', 'parent':'#', 'icon':'/forms/admin/images/classification-tree-root.png' %s }", opt) );
*/									}
									sb.append(" ]");
...
//XXX:2014-11-21				currPrefs.put(cp.getCriterion().getId(), cp);
...
//XXX:2014-11-21					tmp.setCriterion(pv);
...
//XXX:TODO: REMOVE DIRTY HACK :P
logger.trace("VVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV");
ConsumerPreferenceProfile profile = null;
{
eu.brokeratcloud.rest.opt.ProfileManagementService profileMgntWS = new eu.brokeratcloud.rest.opt.ProfileManagementService();
logger.debug("savePreferences: DIRTY HACK to retrieve consumer preference profile: id = {}", profileId);
...
//			String attrId = cp.getCriterion().getAttribute();
//XXX:2014-11-21			String attrId = cp.getCriterion().getRefToServiceAttribute().getId();
...
/*	@GET
	...
	@Produces("application/json")
	public String getProfilePreferences(@PathParam("profile_id") String id) throws IOException {
		long startTm = System.currentTimeMillis();
		logger.info("-------------- getProfilePreferences: INPUT: {}", id);
		
// TODO: call WS to retrieve profile preferences
String str = "[ "+
// ++++++++++++++++++++++++++++++++++++++++++++++++++
				" ]";
str = str.replace("'", "\"");
long callStartTm=0, callEndTm=0;
		...
		logger.info("-------------- getProfilePreferences: OUTPUT: {}", str);
		long endTm = System.currentTimeMillis();
		logger.debug("duration={}ms   contacting WS: {}ms", (endTm-startTm), (callEndTm-callStartTm));
		return str;
	}
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	@Consumes("application/json")
	@Produces("application/json")
	public Response saveProfilePreferences(@PathParam("profile_id") String id) throws IOException {
		long startTm = System.currentTimeMillis();
		logger.info("-------------- saveProfilePreferences: INPUT: {}", id);
		
// TODO: call WS to store profile preferences
long callStartTm=0, callEndTm=0;
		
		logger.info("-------------- saveProfilePreferences: OUTPUT: TODO++++++++++++++++++++++++");
		long endTm = System.currentTimeMillis();
		logger.debug("duration={}ms   contacting WS: {}ms", (endTm-startTm), (callEndTm-callStartTm));
		return Response.status(200).entity("Response:  "+"response.getStatus()"+"\nDuration:  "+(endTm-startTm)).build();
	}*/
...
/*		@XmlAttribute
		...
		@XmlAttribute
		protected String serviceCategoryText;*/
		@XmlAttribute
