...
public class RecommendationsComponent extends AbstractFacingComponent {
	...
	protected Object[] _prepareRecomListJson(String profileId, Recommendation[] recomList) {
		...
		String recomItemFmt = "{'id':'%s','suggestion':'%s','relevance':%.2f,'order':%d,'response':'%s', 'extra':%s}"; //XXX: 2015-01-19: addition of 'extra'
		String recomTail = "]}";
		boolean first = true;
		for (int i=0; i<recomList.length; i++) {
			if (first) first=false; else sb.append(",");
...
//XXX: 2015-01-19: addition of 'extra'
					String extra = extra2str( rit.getExtra() );
					sb.append( String.format(Locale.US, recomItemFmt, iid, suggestion, relevance, j++, response, extra /*XXX: 2015-01-19: addition of 'extra' */) );
				}
			}
